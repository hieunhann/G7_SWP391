...
const RegisteredMembers = () => {
  ...
  if (loading) {
    return <div className="flex flex-col items-center justify-center py-8"><div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-blue-500 mb-2"></div><span>Đang tải dữ liệu...</span></div>;
  }
  if (error) {
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8">
        <h2 className="text-3xl font-extrabold mb-8 text-center text-blue-700 tracking-wide drop-shadow">Danh sách đăng ký chương trình</h2>
        {/* Search & Filter */}
          ...
          <table className="min-w-full border border-gray-200 bg-white rounded-lg overflow-hidden">
            <thead>
              <tr className="bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800">
                <th className="border px-4 py-2 font-semibold">ID đăng ký</th>
                <th className="border px-4 py-2 font-semibold">ID thành viên</th>
                <th className="border px-4 py-2 font-semibold">Họ và tên</th>
                <th className="border px-4 py-2 font-semibold">Số điện thoại</th>
                <th className="border px-4 py-2 font-semibold">Tên chương trình</th>
                <th className="border px-4 py-2 font-semibold">Xem chi tiết</th>
              </tr>
            </thead>
            <tbody>
              {filteredRegistrations.map((reg) => (
                <tr key={reg.id} className="text-center hover:bg-blue-50 transition-colors">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white rounded-2xl shadow-2xl p-8 min-w-[320px] max-w-[90vw] relative border-2 border-blue-200">
            ...
            <div className="flex flex-col items-center mb-4">
              ...
              <h3 className="text-xl font-bold mb-2 text-blue-700">Thông tin thành viên</h3>
            </div>
            {memberLoading ? (
              <div className="flex flex-col items-center"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-blue-500 mb-2"></div>Đang tải...</div>
            ) : memberError ? (
              <div className="text-red-500 flex items-center gap-2"><FaExclamationTriangle />{memberError}</div>
            ) : selectedMember ? (
