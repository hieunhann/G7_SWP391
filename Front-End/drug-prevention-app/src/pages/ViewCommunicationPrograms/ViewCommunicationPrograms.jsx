...
const ViewCommunicationPrograms = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-dark text-white py-5 px-3 px-md-5">
      <h2 className="text-center text-info mb-5 display-4 fw-bold border-bottom pb-3 shadow-lg">
        <i className="bi bi-globe"></i> Danh sách chương trình cộng đồng
      </h2>
      <div className="d-flex flex-wrap justify-content-center gap-4">
        {events.map((event) => (
          <Card key={event.ID} className="event-card border border-info shadow-lg" style={{ maxWidth: "320px" }}>
            {event.Image && (
            ...
            <Card.Body className="bg-dark text-white d-flex flex-column justify-content-between">
              <div>
                <Card.Title className="fs-5 fw-bold text-info d-flex justify-content-between align-items-center">
              ...
              <Button variant="outline-info" className="mt-3 w-100 align-self-end d-flex align-items-center justify-content-center gap-2" onClick={() => handleViewDetail(event)}>
          ...
          <Modal.Body className="bg-light-subtle">
            ...
            {feedbacks.length === 0 ? (
              ...
              <ListGroup variant="flush">
                {feedbacks.map((fb) => (
                  <ListGroup.Item key={fb.ID} className="bg-white border-1 mb-1 rounded">
